...
Run the full Designpowers inclusive-design pipeline, director-driven. This workflow
is the orchestrator: it sequences the ten agent Skills, surfaces handoffs to the
user, and enforces the v2 invariant (measure, don't assert) via the truth-layer.
It holds no design judgment itself — each Skill loads that from the shared core.

## Before anything: the welcome + router

The shared core's entry point governs the start of every session. Read and follow
`skills/using-designpowers/SKILL.md` — show the welcome (the bird), check for a
returning user, and route Build vs Review. Do **not** skip it. The pipeline below
is the Build lane; for "review something that already exists," follow the Review
lane (`skills/design-review/SKILL.md`) instead.
...
creative director and overrides everything. Switch to **Auto** if they say "go
auto" (still surface narration; still pause on the safeguards below).

## The pipeline (Build lane)

Dispatch each agent by invoking its Skill. Each Skill loads its persona +
`core/agents/<id>/contract.md` and works. Keep `design-state.md` current
throughout.

1. **design-strategist** — principles, flows, IA, journey map, persona-to-goal.
2. **design-scout** — competitive/inclusion research evidence (parallel-ok with 1).
...
11. **synthetic-user-testing**, then **verification-before-shipping** (per the core
    skills) — evidence, not "I think it works."
...
- accessibility-critic finds a **critical** (a pair fails AA).
...
`/verify-accessibility-tools` and fix the wiring (see SETUP.md) before proceeding —
never let an agent estimate a ratio.
